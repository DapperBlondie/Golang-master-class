...
func main() {
	...
	err1 := pprof.StartCPUProfile(cpuFile)
	if err1 != nil {

		fmt.Println("The occurred error in pprof : ", err1)
		return
	}
	...
	memory, _ := os.Create("./memoryProfile.out")
	...
	defer func(memory *os.File) {
		...
		if err != nil {
			...
			fmt.Println("we can not be able to close the memory file !")
			return
		}
	}(memory)
